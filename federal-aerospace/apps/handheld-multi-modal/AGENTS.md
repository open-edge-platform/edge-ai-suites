...
The Federal and Aerospace AI Suite Application Blueprint is defined and deployed using Docker Compose files, and combines LLM inference capability served through the OpenVINO™ model server platform, speech-to-text transcription through the Whisper service, a chat UI through the Open WebUI software, and metrics information through the Grafana dashboard. The Blueprint runs with the Visual Pipeline and Platform Evaluation Tool for pipeline visualization on Intel® edge hardware.
