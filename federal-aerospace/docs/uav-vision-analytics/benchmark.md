...
### `jq: command not found` (line ~581)

`jq` is not installed on the host. Options:

```bash
# Option 1: install via apt (requires sudo)
sudo apt-get install -y jq

# Option 2: create docker exec wrapper (no root needed)
mkdir -p ~/.local/bin
cat > ~/.local/bin/jq << 'EOF'
#!/usr/bin/env bash
CONTAINER="dlstreamer-pipeline-server"
...
The `dlstreamer-pipeline-server` container is not running or the stack is not up. Start it:

```bash
make pymav-up
```

Check container status: `docker ps`. If the port mapping differs, override:
...
The `-p` name does not match any entry. List available names:
...
- **DLSPS pipeline in ERROR state** — often a shared `model-instance-id` from a previous aborted run. Restart the container:
  ```bash
  docker restart dlstreamer-pipeline-server
  ```
- **RTSP path conflict** — a previous ABORTED run still holds the RTSP path. The script uses per-run timestamps to prevent this; restart DLSPS to clear leftover registrations.
- **Video file missing inside container** — verify:
  ```bash
