...
description: "On-demand security review for application code, container artifacts, and Kubernetes configuration. Use when a change touches authentication or authorization logic, input parsing or file handling, secrets or logging, dependency upgrades, or any Dockerfile / Docker Compose / Helm chart / CI-CD workflow that could introduce privilege elevation. Covers secure code review such as input validation and injection and dynamic execution and dependency hygiene, guardrails for AI-generated code, container artifact hardening for Dockerfile and Compose, and Helm and Kubernetes security defaults. Produces classified findings that separate what to fix in the artifact from deployment-time responsibility, each with a severity and confidence level. Do NOT use it for runtime or host or cluster or organizational security controls, for general code quality or style reviews unrelated to security, or for enforcing network policies and node hardening and cloud IAM configuration."
---
