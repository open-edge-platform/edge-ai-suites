...
description: "On-demand security review for application code, container artifacts, and Kubernetes configuration. Use it when a change touches authentication or authorization logic; input parsing or file handling; secrets or logging; dependency upgrades; or any Dockerfile, Compose file, Helm chart, or CI/CD workflow that could introduce privilege elevation. It covers secure code review for input validation and injection and dynamic execution and dependency hygiene; guardrails for AI-generated code; container artifact hardening; and Helm and Kubernetes security defaults. Findings are classified as fix-in-artifact versus deployment-time responsibility and carry severity and confidence levels. Do NOT use it for runtime or host or cluster or organizational security controls; for general code-quality or style reviews unrelated to security; or for enforcing network policies and node hardening and cloud IAM configuration."
---
