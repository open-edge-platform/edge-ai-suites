...
class TaskService:
    ...
    @staticmethod
    async def handle_file_upload(db: Session, minio_payload: dict):
        import traceback
        try:
            task = task_crud.create_task(db, "file_search", minio_payload, status=TaskStatus.QUEUED)
            t_id = str(task.id)
            msg = RedisTaskMessage(task_id=t_id)

            redis_client.xadd("stream:file_search", {"task_id": t_id})
            return TaskResponse(
                task_id=str(t_id),
                status=TaskStatus.QUEUED,
                mode="upload_async",
            ...
            print("\n" + "="*50)
            print("❌ DETECTED ERROR IN handle_file_upload:")
            traceback.print_exc()
            print("="*50 + "\n")
            raise e
