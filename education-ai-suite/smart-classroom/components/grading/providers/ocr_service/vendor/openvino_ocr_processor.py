...
from . import openvino_pipeline as pipeline
...
class OpenVINOOCRProcessor(BaseOCR):
    ...
    def _load_char_dict(self):
        ...
        yml_path = self.ir_models_dir / "rec" / self.rec_model / "inference.yml"
        if yml_path.exists():
    ...
    def _load_models(self):
        ...
        det_ir = self.ir_models_dir / "det" / self.det_model / "inference.xml"
        ...
        rec_ir = self.ir_models_dir / "rec" / self.rec_model / "inference.xml"
        ...
        if self.use_angle_cls:
            cls_ir = self.ir_models_dir / "cls" / self.cls_model / "inference.xml"
            if cls_ir.exists():
