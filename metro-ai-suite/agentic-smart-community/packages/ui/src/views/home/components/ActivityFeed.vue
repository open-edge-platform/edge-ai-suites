  ...
  <div class="activity-timeline-card section-card compact-card">
    <div class="timeline-header flex-between">
      ...
      <div class="timeline-header-right flex-left">
        <div v-if="showAlertFilter" class="timeline-legend-filter flex-left">
          <label
            v-for="option in alertFilterOptions"
            :key="`legend-${option.value}`"
            class="timeline-legend-filter-item"
            :class="{
              active: selectedAlertFilters.includes(option.value),
            }"
            :style="getAlertFilterStyle(option.value)"
          >
            <input
              class="timeline-legend-filter-input"
              type="checkbox"
              :checked="selectedAlertFilters.includes(option.value)"
              @change="toggleAlertFilter(option.value)"
            />
            <span
              class="timeline-legend-filter-check"
              aria-hidden="true"
            ></span>
          ...
          >
            ...
            <span
              ...
              :class="[
                entry.recordKind,
                {
                  'alert-tone': entry.isAlertMixed || !!entry.alertTone,
                  mixed: entry.isAlertMixed,
              ...
              :style="{
                ...
                ...getTimelineEntryAlertStyle(entry),
              }"
    ...
    <div v-if="showAlertFilter" class="timeline-legend">
      <div
        v-for="option in alertFilterOptions"
        :key="`timeline-legend-${option.value}`"
        class="legend-item flex-left"
      >
        <div
          class="legend-dot"
          :style="getAlertLegendDotStyle(option.value)"
        ></div>
        <span>{{ option.label }}</span>
      </div>
    </div>
  ...
  <div class="history-section section-card">
      ...
      <div class="activity-loading-indicator">
        ...
        <div class="activity-loading-copy">
          <div class="activity-loading-title">
            {{ $t("smartHome.activityLoadingTitle") }}
          </div>
          <div class="activity-loading-tip">
            {{ $t("smartHome.activityLoadingTip") }}
          </div>
        </div>
      </div>
    </div>
    ...
    >
      <div
        ...
        class="history-item"
        :class="[record.recordKind, { active: selectedRecordId === record.id }]"
      >
        ...
        <div class="history-content">
          <div class="history-header flex-between">
            <div class="history-title-wrap flex-between">
              ...
              <div class="history-meta flex-left">
                ...
                <div
                  v-if="record.alertLabel"
                  class="history-chip alert-chip"
                  :style="
                    record.alertType
                      ? getAlertChipStyle(record.alertType)
                      : undefined
                  "
                >
                  {{ record.alertLabel }}
          ...
          <div class="history-preview-wrap">
            ...
            >
              <span class="history-preview-placeholder-play" aria-hidden="true">
...
const selectedAlertFilters = ref<AlertFilterValue[]>([]);

const KNOWN_ALERT_CONFIG: Record<
  string,
...
> = {
  danger: {
    ...
    tokens: {
      color:
        "color-mix(in srgb, var(--color-error) 88%, var(--color-errorBg) 12%)",
      ring: "color-mix(in srgb, var(--color-errorBg) 72%, transparent)",
      chipBg: "color-mix(in srgb, var(--color-errorBg) 88%, transparent)",
      chipText:
  ...
  normal_play: {
    ...
    tokens: {
      color:
        "color-mix(in srgb, var(--color-success) 82%, var(--color-successBg) 18%)",
      ring: "color-mix(in srgb, var(--color-successBg) 78%, transparent)",
...
const UNKNOWN_ALERT_TOKENS: AlertVisualTokens[] = [
  {
    color:
      "color-mix(in srgb, var(--color-primary) 84%, var(--color-primaryBg) 16%)",
    ring: "color-mix(in srgb, var(--color-primaryBg) 78%, transparent)",
  ...
  {
    color:
      "color-mix(in srgb, var(--color-warning) 84%, var(--color-warningBg) 16%)",
    ring: "color-mix(in srgb, var(--color-warningBg) 82%, transparent)",
    chipBg: "color-mix(in srgb, var(--color-warningBg) 88%, transparent)",
    chipText:
      "color-mix(in srgb, var(--color-warning) 88%, var(--font-main-color) 12%)",
  },
  {
    color:
      "color-mix(in srgb, var(--color-primary-hover) 82%, var(--color-primaryBg) 18%)",
    ring: "color-mix(in srgb, var(--border-primary) 78%, transparent)",
    chipBg: "color-mix(in srgb, var(--color-primaryBg) 88%, transparent)",
    chipText:
      "color-mix(in srgb, var(--color-primary-hover) 88%, var(--font-main-color) 12%)",
  },
  {
    color:
      "color-mix(in srgb, var(--color-success) 82%, var(--color-successBg) 18%)",
...
const normalizeAlertType = (alertType: CameraTaskRecord["alert"]) => {
  ...
  const normalizedValue = alertType.trim();
...
const formatUnknownAlertLabel = (alertType: string) => {
  const words = alertType
    .split(/[_\s-]+/)
    .map((word) => word.trim())
    .filter(Boolean);
...
const buildAlertLabel = (alertType: CameraTaskRecord["alert"]) => {
  const normalizedAlertType = normalizeAlertType(alertType);
  if (!normalizedAlertType) {
    return "";
  }

  const knownConfig = KNOWN_ALERT_CONFIG[normalizedAlertType];
  if (knownConfig) {
    return knownConfig.label();
  }

  return formatUnknownAlertLabel(normalizedAlertType);
};

const compareAlertTypes = (left: string, right: string) => {
  const leftOrder = KNOWN_ALERT_CONFIG[left]?.order ?? Number.MAX_SAFE_INTEGER;
  const rightOrder =
    KNOWN_ALERT_CONFIG[right]?.order ?? Number.MAX_SAFE_INTEGER;

  if (leftOrder !== rightOrder) {
    return leftOrder - rightOrder;
  }

  return buildAlertLabel(left).localeCompare(buildAlertLabel(right));
};

const datedAlertTypes = computed(() => {
  return props.tasks
...
const sortedAlertTypes = computed(() => {
  return datedAlertTypes.value
    .filter(
      (alertType, index, alertTypes) => alertTypes.indexOf(alertType) === index,
    )
    .sort(compareAlertTypes);
});

const unknownAlertOrderMap = computed(() => {
  return new Map(
    sortedAlertTypes.value
      .filter((alertType) => !KNOWN_ALERT_CONFIG[alertType])
      .map((alertType, index) => [alertType, index]),
  );
});

const getAlertVisualTokens = (alertType: string): AlertVisualTokens => {
  const knownConfig = KNOWN_ALERT_CONFIG[alertType];
  if (knownConfig) {
    return knownConfig.tokens;
  }

  const orderIndex = unknownAlertOrderMap.value.get(alertType) ?? 0;
  return UNKNOWN_ALERT_TOKENS[orderIndex % UNKNOWN_ALERT_TOKENS.length];
};

const getUniqueAlertTypes = (records: Pick<ActivityRecord, "alertType">[]) => {
  return [
    ...new Set(
      records
        .map((record) => record.alertType)
        .filter((alertType): alertType is string => Boolean(alertType)),
    ),
  ];
};

const getAlertFilterStyle = (alertType: string) => {
  const tokens = getAlertVisualTokens(alertType);
  return {
    "--legend-filter-color": tokens.color,
    "--legend-filter-ring": tokens.ring,
  };
};

const getAlertChipStyle = (alertType: string) => {
  const tokens = getAlertVisualTokens(alertType);
  return {
    "--alert-chip-bg": tokens.chipBg,
    "--alert-chip-color": tokens.chipText,
  };
};

const getAlertLegendDotStyle = (alertType: string) => {
  const tokens = getAlertVisualTokens(alertType);
  return {
    "--legend-dot-color": tokens.color,
  };
};

const getTimelineEntryAlertStyle = (entry: TimelineEntry) => {
  if (entry.isAlertMixed) {
    const alertTypes = getUniqueAlertTypes(entry.records)
      .sort(compareAlertTypes)
      .slice(0, 2);
    const primaryTokens = getAlertVisualTokens(alertTypes[0]);
    const secondaryTokens = getAlertVisualTokens(
      alertTypes[1] || alertTypes[0],
    );

    return {
      "--alert-segment-color": primaryTokens.color,
      "--alert-segment-ring": primaryTokens.ring,
      "--alert-segment-bg": `linear-gradient(90deg, ${primaryTokens.color} 0 50%, ${secondaryTokens.color} 50% 100%)`,
    };
  }
  ...
  const tokens = getAlertVisualTokens(entry.alertTone);
  return {
    "--alert-segment-color": tokens.color,
    "--alert-segment-ring": tokens.ring,
    "--alert-segment-bg": tokens.color,
  };
};

const alertFilterOptions = computed<AlertFilterOption[]>(() => {
  return sortedAlertTypes.value.map((alertType) => ({
    value: alertType,
    label: buildAlertLabel(alertType),
  }));
});
...
const mapTaskToRecord = (task: CameraTaskRecord): ActivityRecord => {
  ...
  return {
    ...
    alertType,
    alertLabel: buildAlertLabel(alertType),
  };
};
...
const matchesAlertFilter = (record: ActivityRecord) => {
  if (!showAlertFilter.value) {
    return true;
  }
...
const visibleRecords = computed(() => {
  return datedMotionRecords.value.filter(matchesAlertFilter);
});
...
const getAlertTone = (records: ActivityRecord[]) => {
  const alertTypes = getUniqueAlertTypes(records);

  return {
    alertTone: alertTypes.length === 1 ? alertTypes[0] : null,
    isAlertMixed: alertTypes.length > 1,
  };
};
...
const groupedTimelineEntries = computed<TimelineEntry[]>(() => {
  ...
  return [...bucketMap.entries()].map(([bucketStartMinutes, records]) => {
    const { alertTone, isAlertMixed } = getAlertTone(records);
...
const toggleAlertFilter = (filterValue: AlertFilterValue) => {
  if (selectedAlertFilters.value.includes(filterValue)) {
    selectedAlertFilters.value = selectedAlertFilters.value.filter(
      (value) => value !== filterValue,
    );
    return;
  }

  selectedAlertFilters.value = [...selectedAlertFilters.value, filterValue];
};
...
watch(
  ...
  (options, previousOptions) => {
    ...
    const retainedValues = selectedAlertFilters.value.filter((value) =>
    ...
    selectedAlertFilters.value = [
      ...new Set([...retainedValues, ...addedValues]),
    ];
  },
  { immediate: true },
);
...
.activity-timeline-card {
  --timeline-danger-color: color-mix(
    in srgb,
    var(--color-error) 88%,
    var(--color-errorBg) 12%
  );
  --timeline-danger-ring: color-mix(
    in srgb,
    var(--color-errorBg) 72%,
    transparent
  );
  --timeline-normal-color: color-mix(
    in srgb,
    var(--color-success) 82%,
    var(--color-successBg) 18%
  );
  --timeline-normal-ring: color-mix(
    in srgb,
    var(--color-successBg) 78%,
    transparent
  );
}
...
.timeline-legend-filter-item {
  --legend-filter-color: var(--border-primary);
  --legend-filter-ring: transparent;
  display: inline-flex;
  align-items: center;
  gap: 5px;
  position: relative;
  ...
  border-radius: 999px;
  background: color-mix(in srgb, var(--surface-card-bg) 72%, transparent);
  font-size: 10px;
  font-weight: 600;
  ...
  &:hover {
    ...
    border-color: color-mix(
      in srgb,
      var(--legend-filter-color) 44%,
      transparent
    );
    background: color-mix(
      in srgb,
      var(--surface-card-bg-hover) 76%,
      transparent
    );
  }
  ...
  &.active {
    ...
    border-color: color-mix(
      in srgb,
      var(--legend-filter-color) 72%,
      transparent
    );
    background: color-mix(
      in srgb,
      var(--legend-filter-ring) 84%,
      var(--surface-card-bg) 16%
    );
    box-shadow: 0 8px 16px
      color-mix(in srgb, var(--legend-filter-ring) 48%, transparent);
  }

  &.danger {
    --legend-filter-color: var(--timeline-danger-color);
    --legend-filter-ring: var(--timeline-danger-ring);
  }

  &.normal_play {
    --legend-filter-color: var(--timeline-normal-color);
    --legend-filter-ring: var(--timeline-normal-ring);
  }
}

.timeline-legend-filter-input {
  position: absolute;
  opacity: 0;
  pointer-events: none;
}

.timeline-legend-filter-check {
  position: relative;
  width: 11px;
  height: 11px;
  flex: none;
  border-radius: 4px;
  border: 1.5px solid
    color-mix(in srgb, var(--legend-filter-color) 74%, transparent);
  background: color-mix(in srgb, var(--surface-panel-bg) 82%, transparent);
  box-shadow: inset 0 0 0 1px
    color-mix(in srgb, var(--surface-card-bg) 68%, transparent);
  transition:
    border-color 0.2s ease,
    background 0.2s ease,
    box-shadow 0.2s ease,
...
.timeline-legend-filter-check::after {
  content: "";
  position: absolute;
  left: 2px;
  top: 0px;
...
.timeline-legend-filter-item.active .timeline-legend-filter-check {
  border-color: var(--legend-filter-color);
  background: var(--legend-filter-color);
  box-shadow: 0 0 0 3px
    color-mix(in srgb, var(--legend-filter-ring) 90%, transparent);
}

.timeline-legend-filter-item.active .timeline-legend-filter-check::after {
  opacity: 1;
}
...
.activity-loading-indicator {
  ...
  border-radius: 16px;
  background: color-mix(in srgb, var(--surface-panel-bg) 92%, transparent);
  box-shadow: 0 10px 22px
    color-mix(in srgb, var(--bg-box-shadow) 78%, transparent);
}
...
.activity-loading-icon {
  ...
  animation: activity-loading-spin 0.9s linear infinite;
}
...
.timeline-segment.motion.alert-tone {
  background: var(
    --alert-segment-bg,
    var(--timeline-motion-color, var(--color-primary))
  );
  border-color: var(
    --alert-segment-color,
    var(--timeline-motion-color, var(--color-primary))
  ...
  box-shadow: 0 0 0 2px
    var(
      --alert-segment-ring,
      var(--timeline-motion-ring, var(--border-primary))
    );
}
...
.timeline-segment.motion.alert-tone:hover {
  box-shadow:
    0 0 0 3px
      var(
        --alert-segment-ring,
...
.timeline-segment.motion.alert-tone.active {
  background: var(
    --alert-segment-bg,
    var(--timeline-motion-color, var(--color-primary))
  );
...
.legend-dot {
  --legend-dot-color: var(--timeline-motion-color, var(--color-primary));
  width: 10px;
  height: 10px;
  border-radius: 3px;
  background: var(--legend-dot-color);
  border: 1px solid var(--legend-dot-color);
}
...
@keyframes activity-loading-spin {
  100% {
...
.history-chip.alert-chip {
  background: var(--alert-chip-bg, var(--surface-panel-bg));
  color: var(--alert-chip-color, var(--font-main-color));
}
