...
- a different span → `type=weekly` / `type=monthly`, or `type=custom` with `period_start` + `period_end` (`YYYY-MM-DD`, or `YYYY-MM-DD HH:MM` for a half-day window).
...
- Web-search near the user's home address; recommend 1–2 good options (name, rough distance, hours, highlights).
