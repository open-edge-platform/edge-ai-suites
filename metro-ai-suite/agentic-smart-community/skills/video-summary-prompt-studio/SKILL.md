...
| "Delete/remove the `<uc>` use case" | Destructive — confirm first, then `smartbuilding_use_case_register action=unregister persist=true`. Removes the VLM task + `use_case_dict` entry + config.yaml entry, detaches every monitor on this use case (stop worker, delete analytics source, DB row offline, stripped from monitors.yaml), and archives `use-cases/<uc>/` to `use-cases/.backup/`. Any incomplete cleanup → `degraded:true` (check warnings); `skipped` + "still referenced" warning means another use case shares the task and is not degraded |
