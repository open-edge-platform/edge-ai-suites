...
```bash
helm pull oci://registry-1.docker.io/intel/live-video-alert-agent --version <version-no>
tar -xvf live-video-alert-agent-<version-no>.tgz
cd live-video-alert-agent
```
