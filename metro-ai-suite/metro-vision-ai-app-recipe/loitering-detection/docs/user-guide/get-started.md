...
- In Grafana UI, the dashboard displays detected people and cars
    ...
    > **Note:**  In the default pipeline, we use `gvatrack tracking-type=short-term-imageless` element. `gvatrack` supports two imageless tracking modes: `zero-term-imageless` and `short-term-imageless`. Zero-term assigns IDs per frame and requires detection on every frame. Short-term tracks objects across frames using motion and shape, reducing the need for continuous detection and improving FPS. Since it does not use image features, the same object may receive different IDs over time due to lack of re-identification.
