...
Follow the [Trusted Compute baremetal installation guide](https://docs.openedgeplatform.intel.com/trusted-compute/baremetal-installation) to install Trusted Compute version 1.5.2 or later on your host system. Complete the following sections:
...
> **Note:** All other setup and configuration steps remain the same as described in the [Setup and First Use](#setup-and-first-use) section above.
...
- Trusted Compute version 1.5.2 or later
...
##### Step 4: Deploy with GPU Enabled
...
The DL Streamer Pipeline Server containers will run inside a Trusted Compute VM with GPU passthrough enabled, providing both hardware isolation and GPU-accelerated inference.
...
If you deployed with GPU passthrough, unbind the GPU from vfio-pci to restore it to the host:
