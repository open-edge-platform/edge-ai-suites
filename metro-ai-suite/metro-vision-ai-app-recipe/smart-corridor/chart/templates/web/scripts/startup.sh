...
sed -i "s/'HOST': 'localhost'/'HOST':'smart-corridor-pgserver'/g" /home/scenescape/SceneScape/manager/settings.py &&
...
/usr/local/bin/scenescape-init webserver --dbhost smart-corridor-pgserver --dbtype postgres --dbport "5432" --broker broker.scenescape.intel.com --brokerauth /run/secrets/browser.auth --brokerrootcert /run/secrets/certs/scenescape-ca.pem
{{- end -}}