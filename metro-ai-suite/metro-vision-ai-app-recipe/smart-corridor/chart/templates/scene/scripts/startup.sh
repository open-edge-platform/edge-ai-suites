...
echo $SMART_CORRIDOR_VDMS_SERVICE_HOST    vdms.scenescape.intel.com >> /etc/hosts &&
...
cp /tmp/secrets_/scenescape-vdms-c.crt /tmp/secrets/scenescape-vdms-c.crt &&
cp /tmp/secrets_/scenescape-vdms-c.key /tmp/secrets/scenescape-vdms-c.key &&
