...
function run_sample() {
  ...
  if [ $device == "GPU" ]; then
    ...
    pipeline_name="yolov11s_igpu_cpu"
  elif [ $device == "IGPU_NPU_CPU" ]; then
    pipeline_name="yolov11s_igpu_npu_cpu"
  else
...
if $forcedGPU; then
  ...
  if ls /dev/dri/renderD* 1> /dev/null 2>&1; then
    echo -e "\n>>>>>iGPU+CPU mode selected (iGPU decode and detection, CPU classification)."
  ...
  if ls /dev/dri/renderD* 1> /dev/null 2>&1; then
    if ls /dev/accel/accel* 1> /dev/null 2>&1; then
      echo -e "\n>>>>>iGPU+NPU+CPU mode selected (iGPU decode and detection, NPU classification)."
