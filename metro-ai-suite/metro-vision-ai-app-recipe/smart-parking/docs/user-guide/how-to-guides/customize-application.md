...
further analysis (like calculating distances), and publishes results.
...
> For detailed documentation on DL Streamer Pipeline Server, visit the [DL Streamer Pipeline Server Documentation](https://docs.openedgeplatform.intel.com/2026.0/edge-ai-libraries/dlstreamer-pipeline-server/index.html).
...
Pipelines like `yolov11s`, `yolov11s_gpu`, `yolov11s_npu` etc., are used to identify objects
in the video frames.
...
Pipelines like `object_tracking_cpu`, `object_tracking_gpu`, `object_tracking_npu` are designed
to track objects detected by the object detection pipelines.
...
frames, which is essential for smart parking and other advanced analytics.
...
smart parking. Using a drag-and-drop interface, you can build complex workflows with
...
-   **Configuration Details:**
    ...
    -   `Topics: object_tracking_1, object_tracking_2` (etc., as an example)
...
-   **Key Tasks:**
    ...
    -   **Filtering and Timestamping:** Iterates through the object list, applying filters
...
-   **Installation:** Install Grafana on your local machine or server by following the official
...
-   **Step-by-Step (Example with MQTT):**
    ...
    4.  Save and test the connection to ensure it is working.
...
datasource to create dashboards showing real-time object counts and events.
...
the calculated `object detection` flag and any other derived metrics.

## Conclusion

This guide has demonstrated a complete object tracking and smart parking system using
