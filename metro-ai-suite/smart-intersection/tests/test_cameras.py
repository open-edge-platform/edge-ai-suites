...
def test_add_camera(waiter):
  ...
  sensor_id_input.send_keys(id_of_new_camera)
  id_name_input.send_keys(name_of_new_camera)
  ...
  camera_card = waiter.wait_and_assert(
      EC.visibility_of_element_located((By.ID, f"rate-{id_of_new_camera}")),
      error_message=f"Camera card with ID 'rate-{id_of_new_camera}' is not visible on the page"
  )  
