...
> **Note:** All other setup and configuration steps remain the same as described in the [Quick Start with Setup Script](#quick-start-with-setup-script) section above.
...
The DL Streamer Pipeline Server and OpenVINO model server (OVMS) containers will run inside hardware-isolated TC VMs, protecting inference workloads and video data from untrusted co-tenants on the same host.
...
##### Step 3: Deploy with GPU Enabled
...
The OVMS container will run inside a Trusted Compute VM with GPU passthrough enabled, providing both hardware isolation and GPU-accelerated inference.
