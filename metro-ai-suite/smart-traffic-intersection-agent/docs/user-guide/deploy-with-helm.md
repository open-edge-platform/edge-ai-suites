...
- The cluster must support **dynamic provisioning of Persistent Volumes (PV)** for the VLM model cache. Refer to the [Kubernetes Dynamic Provisioning Guide](https://kubernetes.io/docs/concepts/storage/dynamic-provisioning/) for more details.
...
The chart deploys services as `NodePort` by default. Kubernetes assigns random NodePorts. Retrieve the allocated ports and a node IP:
...
| `trafficAgent.mqtt.host` | MQTT broker hostname. Uses fully qualified K8s DNS so the agent works across namespaces. | `smart-intersection-broker.smart-intersection.svc.cluster.local` |
...
| `vlmServing.env.modelName` | Hugging Face model identifier. The default is the validated model for traffic scene analysis; override with any compatible vision-language model. | `microsoft/Phi-3.5-vision-instruct` |
...
Create a file called `values-override.yaml` with the following content, adjusting values for your environment:

```yaml
# values-override.yaml — example overrides (this file is not part of the chart;
# create it yourself and pass it with -f)
global:
  proxy:
...
To deploy VLM inference on an Intel GPU (the default), create a file called `values-gpu-override.yaml` with the following content:

```yaml
# values-gpu-override.yaml — example GPU overrides (this file is not part of the
# chart; create it yourself and pass it with -f)
vlmServing:
  gpu:
    enabled: true
