...
def check_open_ports(target):
    try:
        ...
        result = common_utils.run_command(['nmap', '-p-', target], capture_output=True, text=True)
...
def check_nmap(target, ports):
    try:
        ...
        result = common_utils.run_command(['nmap', '-p', port_list, target], capture_output=True, text=True)
