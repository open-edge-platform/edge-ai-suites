...
def verify_pods(namespace, timeout=300, interval=5):
    ...
    try:
        while True:
            ...
            result = common_utils.run_command(kubectl_command, capture_output=True, text=True, check=True)
...
def validate_helm_deployment_resources(namespace, cpu_threshold_millicores=2000, memory_threshold_gb=8):
    ...
    try:
        completed = common_utils.run_command(metrics_command, capture_output=True, text=True, check=True)
...
def verify_multimodal_core_components(namespace):
    ...
    try:
        completed = common_utils.run_command(
...
def uninstall_helm_charts(release_name, namespace):
    ...
    try:
        ...
        result = common_utils.run_command(list_command, capture_output=True, text=True, check=True)
        ...
        if release_name in releases:
            ...
            uninstall_result = common_utils.run_command(uninstall_command, capture_output=True, text=True, check=True)
...
def list_directory_contents():
    ...
    try:
        ...
        common_utils.run_command(["ls", "-al"], check=True)
    except subprocess.CalledProcessError as e:
...
def wait_for_mqtt_sample(namespace, topic=constants.WIND_TURBINE_MQTT_TOPIC, timeout=180, interval=10):
    ...
    while time.time() < deadline:
        ...
        result = common_utils.run_command(command, capture_output=True, text=True)
...
def verify_mqtt_alerts_via_subscription(namespace, alert_type, timeout=180, interval=15):
    ...
    while time.time() < deadline:
        ...
        result = common_utils.run_command(command, capture_output=True, text=True, timeout=interval + 5)
        stdout = result.stdout.strip()
...
def execute_influxdb_commands(namespace, chart_path, sample_app=constants.WIND_SAMPLE_APP):
    ...
    try:
        ...
        def _run_influx(query):
            ...
            return common_utils.run_command(command, capture_output=True, text=True, check=True).stdout
...
def verify_multimodal_influxdb_data(chart_path, namespace=None, database=constants.INFLUXDB_DATABASE):
    ...
    pod_proc = common_utils.run_command(
        ["kubectl", "get", "pods", "-n", namespace, "-o", "json"],
        capture_output=True,
        text=True,
    )
    ...
    def _exec_influx(query):
        ...
        return common_utils.run_command(exec_command, capture_output=True, text=True, check=True).stdout
...
def query_influxdb_measurement_via_kubectl(
    ...
    try:
        proc = common_utils.run_command(cmd, capture_output=True, text=True, timeout=timeout)
...
def generate_helm_chart_targz(chart_path, sample_app=constants.WIND_SAMPLE_APP):
    ...
    try:
        ...
        if is_multimodal:
            ...
            common_utils.run_command(["mkdir", "-p", "helm-packages"], check=True)
            pkg_result = common_utils.run_command(
...
def helm_install(release_name, chart_path, namespace, telegraf_input_plugin, continuous_simulator_ingestion="True", val="false", sample_app=None):
    ...
    try:
        ...
        result = common_utils.run_command(helm_command, capture_output=True, text=True, check=True)
        logger.info(result.stdout)
...
def restart_deployment(namespace, pod):
    ...
    try:
        ...
        common_utils.run_command(["kubectl", "get", "deployments", "-n", namespace], check=True)
...
def setup_multimodal_udf_deployment_package(chart_path, namespace, device_value="cpu"):
    ...
    try:
        ...
        if os.path.exists(models_path):
            ...
            result = common_utils.run_command(kubectl_cp_dlstreamer, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            if result.returncode == 0:
        ...
        for item in ["models", "tick_scripts", "udfs"]:
            if os.path.exists(item):
                result = common_utils.run_command(['cp', '-r', item, 'weld_defect_detector/.'], capture_output=True, text=True)
                if result.returncode == 0:
        ...
        result = common_utils.run_command(kubectl_cp_ts, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        if result.returncode == 0:
        ...
        result = common_utils.run_command(dlstreamer_activate_command, capture_output=True, text=True, timeout=30)
        if result.returncode == 0:
...
def copy_dlstreamer_models_to_pod(chart_path, namespace):
    ...
    try:
        ...
        result = common_utils.run_command(kubectl_cp, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        if result.returncode != 0:
            logger.error(f"Error copying DL Streamer models: {result.stderr.decode('utf-8')}")
            return False
        logger.info("✓ DL Streamer models copied successfully.")
...
def cleanup_failed_dlstreamer_pipelines(namespace):
    ...
    try:
        ...
        result = common_utils.run_command(status_command, capture_output=True, text=True, timeout=10)
        ...
        for instance in status_list:
            ...
            if state in ["ERROR", "ABORTED"] and instance_id:
                ...
                delete_result = common_utils.run_command(delete_command, capture_output=True, text=True, timeout=10)
...
def activate_multimodal_dlstreamer_pipeline(
    ...
    try:
        result = common_utils.run_command(activate_command, capture_output=True, text=True, timeout=30)
    except subprocess.SubprocessError as exc:
...
def force_cleanup_namespace(namespace):
    ...
    for command in cleanup_commands:
        try:
            result = common_utils.run_command(command, capture_output=True, text=True, check=False)
...
def execute_gpu_config_curl_helm(
    ...
    try:
        ...
        result = common_utils.run_command(get_pod_cmd, capture_output=True, text=True, timeout=30)
...
def get_vision_img_handles_from_influxdb_helm(credentials, namespace, database="datain", measurement="vision-weld-classification-results"):
    ...
    try:
        ...
        result = common_utils.run_command(kubectl_cmd, capture_output=True, text=True, timeout=30)
...
def execute_seaweedfs_bucket_query_helm(namespace):
    ...
    try:
        ...
        result = common_utils.run_command(curl_cmd, capture_output=True, text=True, timeout=45)
...
def validate_s3_images_content_helm(namespace, matched_files, max_files_to_check=3):
    ...
    try:
        ...
        for file_path in files_to_check:
            ...
            result = common_utils.run_command(curl_cmd, capture_output=True, text=True, timeout=20)
