...
def _run_gpu_helm_test(request):
    ...
    assert_condition(result is True, "Failed to verify pods.")
    ...
    assert_condition(result is True, "Failed to activate UDF deployment package.")
    ...
    assert_condition(pods_ok is True, "Pods are not healthy after GPU configuration update")
