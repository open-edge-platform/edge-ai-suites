...
def test_authentication_influx_grafana(setup_helm_environment, telegraf_input_plugin):
    ...
    assert_condition(pods_result == True, "Failed to verify pods in the namespace.")
    ...
    assert_condition(setup_result == True, "Failed to activate UDF deployment package.")
    ...
    assert_condition(ts_logs_result is True, "Failed to verify pod logs for OPC-UA input plugin.")
    # Access the test cases dictionary
    ...
    assert_condition(influxdb_login_result == True, "Failed to login to InfluxDB with provided credentials.")
    ...
    assert_condition(grafana_login_result == True, "Failed to login to Grafana with provided credentials.")
...
def test_nmap_open_ports_opcua(setup_helm_environment, telegraf_input_plugin):
    ...
    assert_condition(nmap_result == True, "Failed to find open ports on the target.")
...
def test_nmap_open_ports_mqtt(setup_helm_environment, telegraf_input_plugin):
    ...
    ts_logs_result = helm_utils.verify_ts_logs(namespace, "DEBUG")
    logger.info(f"verify_ts_logs result: {ts_logs_result}")
    assert_condition(ts_logs_result is True, "Failed to verify pod logs for MQTT input plugin.")
    exposed_ports = security_utils.find_exposed_ports_helm(namespace)
    nmap_result = security_utils.check_nmap(target, exposed_ports)
    logger.info(f"check_nmap result: {nmap_result}")
    assert_condition(nmap_result == True, "Failed to find open ports on the target.")
...
def test_creds_in_pod_logs(setup_helm_environment, telegraf_input_plugin):
    ...
    assert_condition(pods_creds_result == True, "Credentials found in pod logs.")
...
def test_data_integrity():
    ...
    assert_condition(docker_file_integrity_result == True, "Docker-compose file integrity verification failed.")
    ...
    assert_condition(helm_file_integrity_result == True, "Helm files integrity verification failed.")
    ...
    assert_condition(check_pods_result == True, "Pods are still running after cleanup.")
    ...
    assert_condition(update_yaml_result == True, "Failed to update values.yaml.")
    ...
    assert_condition(helm_install_result == True, "Failed to install Helm release.")
    ...
    assert_condition(pods_result_2 is True, "Failed to verify pods for Case 3.")
    ts_logs_result = helm_utils.verify_ts_logs(namespace, "DEBUG")
    logger.info(f"verify_ts_logs result: {ts_logs_result}")
    assert_condition(ts_logs_result is True, "Failed to verify pod logs for MQTT input plugin.")
    first_wind_speed, last_wind_speed, total_records = security_utils.fetch_wind_turbine_data(chart_path)
    assert_condition(first_wind_speed is not None or last_wind_speed is not None or total_records is not None, "Failed to fetch wind turbine data.")
    ...
    assert_condition(data_integrity_result == True, "Data integrity verification failed in InfluxDB.")
    time.sleep(5)
    ...
    assert_condition(uninstall_result_2 == True, "Failed to uninstall Helm release after data integrity test.")
    check_pods_result_2 = helm_utils.check_pods(namespace)
    logger.info(f"check_pods result: {check_pods_result_2}")
    assert_condition(check_pods_result_2 == True, "Pods are still running after cleanup.")
...
def test_verify_sensitive_data_in_mqtt_logs(setup_docker_environment):
    ...
    assert_condition(results["success"], f"Credential verification failed at step: {results.get('failed_step', 'unknown')}")
    ...
    assert_condition(results["env_variables_read"], "Failed to read environment variables")
    assert_condition(results["deployment_success"], "Failed to deploy containers")
    assert_condition(results["containers_up"], "Containers are not running")
    assert_condition(results["credentials_in_logs"], "Credentials not found in container logs")
