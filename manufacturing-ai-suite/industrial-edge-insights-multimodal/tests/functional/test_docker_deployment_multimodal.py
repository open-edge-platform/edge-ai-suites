...
def test_multimodal_make_up():
    ...
    assert result == True, "Multimodal 'make up' command failed"
    ...
    for container in multimodal_containers:
        assert docker_utils.container_is_running(container), f"Container {container} is not running"
...
def test_multimodal_make_down(setup_multimodal_environment):
    ...
    for container in multimodal_containers:
        assert docker_utils.container_is_running(container), f"Container {container} is not running. Cannot test teardown."
    ...
    assert result == True, "Multimodal 'make down' command failed"
    ...
    assert len(running_containers) == 0, f"Make down failed to stop all containers. Still running: {running_containers}"
...
def test_time_series_ingested_data(setup_multimodal_environment):
    ...
    for container in required_containers:
        assert docker_utils.container_is_running(container), f"{container} container is not running. Deploy multimodal stack first."
    ...
    assert username and password, "InfluxDB credentials not found in environment"
    ...
    if not result:
        ...
        assert result == True, f"Time series data ingestion failed - no data found in InfluxDB measurement: {ingested_topic}"
    else:
...
def test_time_series_analytics_processing(setup_multimodal_environment):
    ...
    for container in required_containers:
        assert docker_utils.container_is_running(container), f"{container} is not running. Deploy multimodal stack first."
    ...
    if not result:
        ...
        assert result == True, f"Time series analytics processing failed - no processed data found in InfluxDB measurement: {analytics_topic}"
    else:
...
def test_vision_analytics_mqtt_publish(setup_multimodal_environment):
    ...
    assert docker_utils.container_is_running(constants.CONTAINERS["dlstreamer"]["name"]), "DLStreamer container is not running. Deploy multimodal stack first."
...
def test_influxdb_data_storage_multimodal(setup_multimodal_environment):
    ...
    assert docker_utils.container_is_running(constants.CONTAINERS["influxdb"]["name"]), "InfluxDB container not running"
    
    # Get credentials
    credentials = context["credentials"]
    username = credentials.get("INFLUXDB_USERNAME", "")
    password = credentials.get("INFLUXDB_PASSWORD", "")
    assert username and password, "InfluxDB credentials not found"
...
def test_rtsp_streaming(setup_multimodal_environment):
    ...
    assert docker_utils.container_is_running(constants.MEDIAMTX_CONTAINER), "MediaMTX container is not running. Deploy multimodal stack first."
...
def test_webrtc_functionality(setup_multimodal_environment):
    ...
    assert docker_utils.container_is_running(constants.COTURN_CONTAINER), "COTURN container is not running. Deploy multimodal stack first."
...
def test_container_logs_multimodal(setup_multimodal_environment):
    ...
    if "skip_reason" in logs_results:
        ...
        assert False, f"Critical containers are not running after deployment: {logs_results['skip_reason']}"
    
    assert logs_results["success"], f"Critical containers have errors: {logs_results['critical_errors']}"
...
def test_fusion_decision_making_logic_validation(setup_multimodal_environment):
    ...
    assert docker_utils.container_is_running(constants.CONTAINERS["fusion_analytics"]["name"]), "Fusion analytics container is not running. Deploy multimodal stack first."
    ...
    assert validation_results["success"], f"Fusion decision-making logic validation failed: {validation_results.get('error', 'Unknown error')}"
    
    # Additional assertions for key metrics
    assert validation_results["total_decisions"] >= 10, f"Insufficient decisions analyzed: {validation_results['total_decisions']}"
    assert validation_results["consistency_percentage"] >= 100.0, f"Logic consistency below threshold: {validation_results['consistency_percentage']}%"
    assert validation_results["unique_defect_types"] >= 5, f"Insufficient defect type diversity: {validation_results['unique_defect_types']}"
    
    # Verify both systems are contributing
    assert validation_results["vision_anomalies"] > 0, "Vision analytics should detect at least some anomalies"
    assert validation_results["ts_anomalies"] > 0, "Time series analytics should detect at least some anomalies"
    
    # Verify all decision categories are represented
    categorized = validation_results["categorized_cases"]
    assert categorized["both_anomaly"] > 0, "Should have cases where both systems detect anomalies"
    assert categorized["vision_only"] > 0, "Should have vision-only detection cases"
    assert categorized["ts_only"] > 0, "Should have TS-only detection cases"
    assert categorized["no_anomaly"] > 0, "Should have no-anomaly cases"
...
def test_system_resources_multimodal():
    ...
    assert resource_results["success"], f"Containers with excessive resource usage: {resource_results['problematic_containers']}"
...
def test_nginx_proxy_integration(setup_multimodal_environment):
    ...
    assert health_results["container_running"], f"Nginx container not running"
    assert health_results["process_running"], "Nginx process not found"
    
    # Verify port mappings
    port_results = docker_utils.verify_nginx_port_mappings(constants.NGINX_CONTAINER, constants.NGINX_EXPECTED_PORTS)
    assert port_results["success"], f"Port mapping failed: {port_results['errors']}"
    
    # Verify backend services
    assert docker_utils.container_is_running(constants.CONTAINERS["grafana"]["name"]), "Grafana container not running"
    assert docker_utils.container_is_running(constants.CONTAINERS["time_series_analytics"]["name"]), "TS Analytics container not running"
    ...
    assert grafana_results["success"], f"Grafana proxy failed: {grafana_results['errors']}"
    ...
    assert api_results["success"], f"TS API proxy failed: {api_results['errors']}"
    ...
    assert endpoint_results["success"], f"Endpoint validation failed: {endpoint_results['critical_failures']}"
...
def test_s3_stored_images_access(setup_multimodal_environment):
    ...
    if not container_check["success"]:
        ...
        assert False, f"Essential containers not running: {missing}"
    ...
    assert container_check["success"], f"Essential containers not running: {container_check['missing_containers']}"
...
def test_vision_metadata_sender_timestamp(setup_multimodal_environment):
    ...
    assert docker_utils.container_is_running(constants.CONTAINERS["influxdb"]["name"]), "InfluxDB container is not running"
    ...
    username = credentials.get("INFLUXDB_USERNAME")
    password = credentials.get("INFLUXDB_PASSWORD")
    assert username and password, "InfluxDB credentials missing from test context"
    ...
    assert query_result["records"], f"No records returned from measurement {vision_measurement}"
    ...
    assert timestamps, "No RTP sender timestamps found in vision metadata entries"
