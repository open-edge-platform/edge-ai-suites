...
def _run_multimodal_helm_gpu_flow(device):
    ...
    assert_condition(pods_result is True, "Failed to verify pods before UDF activation")
    ...
    assert_condition(step1, "Failed to copy DL Streamer models to pod")
    ...
    assert_condition(step4, f"Failed to activate DL Streamer pipeline on {device_upper}")
