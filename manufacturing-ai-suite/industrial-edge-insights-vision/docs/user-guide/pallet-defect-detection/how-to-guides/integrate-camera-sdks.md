...
```bash
docker build -t intel/dlstreamer-pipeline-server:2026.2.0-ubuntu24-rc2 -f BalluffDockerfile .
```
...
```bash
docker build -t intel/dlstreamer-pipeline-server:2026.2.0-ubuntu24-rc2 -f BaslerDockerfile .
```
...
```bash
DLSTREAMER_PIPELINE_SERVER_IMAGE=intel/dlstreamer-pipeline-server:2026.2.0-ubuntu24-rc2
```
