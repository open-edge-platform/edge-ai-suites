...
This guide walks you through using Intel® Geti™ to create a classification project for Weld Porosity Classification, train a model on your Weld Porosity Classification data, and deploy it in the pipeline.
...
- Images of weld cross-sections or weld beads (normal and porous) for training
...
Click on **Create New Project** to start a new Weld Porosity Classification project.
...
Define the labels for your classification task (e.g., "Good Weld", "Porosity", "No Weld"):
...
Upload your training dataset containing weld images. Include representative samples of both normal welds and porous welds to achieve balanced training.

![Browse and Upload Images](../_assets/browse.png)

### Step 10: Annotate Images

Assign the appropriate label (e.g., `Normal` or `Porous`) to each image. After annotating a sufficient number of images, Geti™ will automatically start training the model.

![Annotate Images](../_assets/annotate.png)

*Note: Image is for illustration purposes only.*

> **Note:** By default, Geti™ selects an appropriate classification model backbone. For more control over your model training, explore the [Advanced Guide](#advanced-guide) section to change the backbone architecture, configure custom training parameters, and apply model optimization techniques (FP16, INT8).
...
Repeat the annotation process with additional images to improve classification accuracy. A diverse and balanced dataset leads to better model generalization.
...
```
resources/models/weld-porosity/deployment/Classification/model/model.bin  <- Replace with downloaded version
resources/models/weld-porosity/deployment/Classification/model/model.xml  <- Replace with downloaded version
```
...
- Continuously improve accuracy by adding more annotated weld images
- Retrain as needed when new weld materials or processes are introduced
