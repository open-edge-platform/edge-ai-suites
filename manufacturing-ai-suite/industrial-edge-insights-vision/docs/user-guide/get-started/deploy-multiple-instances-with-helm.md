...
7. Go to `https://<HOST_IP>:<NGINX_HTTPS_PORT>/storage/buckets/ecgdemo/` to browse the frames stored in the `ecgdemo` bucket. You will be prompted to log in with the `S3_STORAGE_USERNAME`/`S3_STORAGE_PASSWORD` credentials provided in `helm/temp_apps/SAMPLE_APP/INSTANCE_NAME/values.yaml` file (HTTP Basic Auth).
