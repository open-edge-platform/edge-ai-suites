...
3. Update the OPC UA `variable` to appropriate value for the pipeline `pcb_anomaly_detection_opcua` in ``configs/pipeline-server-config.json`` for docker or `helm/config.json` for helm.
