...
To deploy the **PCB Anomaly Detection** Sample Application with the Edge Orchestrator, follow the steps described in this document.
...
1. Clone the **edge-ai-suites** repository and change into PCB Anomaly Detection directory:
...
1. On the Setup a Deployment page, select the **pcb** package for the deployment from the list, and click **Next**. The Select a Profile step appears.
...
The **PCB Anomaly Detection** Sample Application is fully deployed when the applications become green and the status is shown as _Running_.
...
### Access the **PCB Anomaly Detection** Sample Application
...
1. Follow the steps described in the **PCB Anomaly Detection** [Documentation](<how-to-deploy-with-helm.md#run-multiple-ai-pipelines>) on usage of the application.
