...
in [scripts/env.sh](scripts/env.sh) is non-empty (the default). `rviz2`
...
[scripts/env.sh](scripts/env.sh) already defaults `RMW_IMPLEMENTATION` to
...
same-host pub/sub. [scripts/setup_dds_shm.sh](scripts/setup_dds_shm.sh) adds
...
documented in [FAST-LIVO2/README_ROS2.md](FAST-LIVO2/README_ROS2.md) —
[scripts/install_deps.sh](scripts/install_deps.sh) automates exactly those
...
point `UNDERLAY_SETUP` in [scripts/env.sh](scripts/env.sh) at an existing
...
[scripts/env.sh](scripts/env.sh)'s `ntu_viral_datafile_id()`; or just run
...
ground truth — [evaluate_viral.py](FAST-LIVO2/Log/result/ntu_viral/evaluate_viral.py),
...
([FAST-LIVO2/Log/result/ntu_viral/README.md](FAST-LIVO2/Log/result/ntu_viral/README.md)).
A fresh measurement within `RMSE_TOLERANCE_PCT` (20% by default,
[scripts/env.sh](scripts/env.sh)) of it is an expected pass — the check
