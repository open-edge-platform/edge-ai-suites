...
This will:
1. Start Prometheus (metrics database) on port 9091
2. Start Grafana (visualization) on port 3000
...
./monitor_stack.py --session grafana_demo

# Terminal 2: Start exporter reading from session files
./src/prometheus_exporter.py --session-dir monitoring_sessions/grafana_demo
```
...
./src/prometheus_exporter.py --mode live

# Start monitoring with --prometheus flag (requires implementation)
./monitor_stack.py --prometheus
```
...
```bash
# Visit Prometheus UI
firefox http://localhost:9091/targets
...
1. Check available metrics:
   ```bash
   curl http://localhost:9091/api/v1/label/__name__/values | grep ros2
   ```
...
- Port 9091: Prometheus UI (host port mapped in compose)
...
curl -H "Content-Type: application/json" \
  http://admin:admin@localhost:3000/api/dashboards/uid/ros2-kpi-dashboard \
  > my_dashboard.json
```
...
# Start monitoring
./monitor_stack.py --session bag_analysis

# Start exporter
./src/prometheus_exporter.py --session-dir monitoring_sessions/bag_analysis
```
...
./monitor_stack.py --remote-ip 192.168.1.100 --session remote_robot

# Export metrics locally
./src/prometheus_exporter.py --session-dir monitoring_sessions/remote_robot
```
