...
Remapping mode is one of the operating modes in Collaborative SLAM and is used to update the pre-constructed
keyframe/landmark map and octree map in mapping mode. A manual 2D-region input from the user is needed to help
Collaborative SLAM determine which region of the pre-constructed maps is pending to be updated. This remapping region
is a quadrilateral and can be either convex or concave. The detailed shape of this remapping region is determined
by the 4 vertexes input by the user. Collaborative SLAM will construct local maps on the tracker side and merge them
into the loaded global maps on the server side, both for the keyframe/landmark map and the octree map.
...
pre-constructed keyframe/landmark map together with an octree map, you can load them in the localization mode and visualize
them through the server RViz. It will help you determine the remapping region by publishing points in the RViz window.
For saving the keyframe/landmark map in mapping mode, you can refer to [geekplus_doc.md](geekplus_doc.md). For saving the octree map in mapping mode, you
can use the `octree_store_path` parameter when launching the tracker node. The usage of this parameter is the same as in the remapping mode section below.
...
to the same values as in the mapping mode.
...
# Remapping (server)
# Replace the server_remapping_region vertexes with your own
