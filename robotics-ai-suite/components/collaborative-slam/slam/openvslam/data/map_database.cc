...
map_database::map_database() { spdlog::debug("CONSTRUCT: data::map_database"); }
...
void map_database::unregister_map_id(MapID id)
{
  ...
  if (min_available_map_id_ < id) {
    min_available_map_id_ = id;
  }
}
...
void map_database::find_next_available_map_id()
{
  while (true) {
    if (min_available_map_id_ == std::numeric_limits<MapID>::max()) {
      spdlog::critical(
        "map_database: map ID is about to overflow! (now max ID is {})", min_available_map_id_ - 1);
      throw std::overflow_error("map_id about to overflow");
    }
    if (registered_map_ids_.find(min_available_map_id_) == registered_map_ids_.end()) {
      break;
    }
    ++min_available_map_id_;
  }
...
std::vector<keyframe *> map_database::get_all_keyframes(MapID map_id1, MapID map_id2) const
{
  ...
  for (const auto & id_keyframe : keyframes_) {
    if (
      id_keyframe.second->get_map_id() == map_id1 || id_keyframe.second->get_map_id() == map_id2) {
...
void map_database::get_all_keyframes_map(
  std::map<KeyframeID, keyframe *> & all_keyframes_map, MapID map_id) const
{
...
data::keyframe * map_database::get_origin_keyframe(MapID map_id) const
{
  ...
  if (!origin_keyfrms_.count(map_id)) {
    return nullptr;
  }
...
std::vector<landmark *> map_database::get_all_landmarks(MapID map_id1, MapID map_id2) const
{
  ...
  for (const auto & id_landmark : landmarks_) {
    if (
      id_landmark.second->get_map_id() == map_id1 || id_landmark.second->get_map_id() == map_id2) {
...
void map_database::get_all_landmarks_map(
  std::map<LandmarkID, landmark *> & all_landmarks_map, MapID map_id) const
{
  std::scoped_lock<std::mutex> lock(mtx_map_access_);
  for (const auto & id_landmark : landmarks_) {
...
std::vector<landmark *> map_database::get_landmarks_in_frustum(
  Eigen::Matrix4d curr_pose, camera::base * curr_camera, MapID curr_map_id, KeyframeID curr_kf_id,
  bool kf_id_is_valid, std::vector<cv::Point3f> & point_3d, std::vector<cv::Point2f> & point_2d,
  std::vector<cv::Point2f> & convex_hull, double near, double far, double back)
{
  ...
  auto pose_can_observe = [&curr_pose, &curr_camera](
                            data::landmark * lm, float x_expand, float y_expand) {
    const Vec3_t pos_w = lm->get_pos_in_world();
    Vec2_t reproj;
    float right_x;
    return curr_camera->reproject_to_image_bound(
      curr_pose.block(0, 0, 3, 3), curr_pose.block(0, 3, 3, 1), pos_w, reproj, right_x, x_expand,
      y_expand);
  };
  ...
  for (int x = min_grid_x; x <= max_grid_x; ++x) {
    for (int y = min_grid_y; y <= max_grid_y; ++y) {
      ...
      for (auto & item : landmarks_in_grid) {
        ...
        if (!item.second || item.second->get_map_id() != curr_map_id) {
          continue;
        }
        ...
        if (kf_id_is_valid && (curr_kf_id <= last_obs_id + 3) && (last_obs_id <= curr_kf_id + 3)) {
          continue;
        }
...
std::vector<landmark *> map_database::get_landmarks_in_frustum(
  keyframe * camera_frame, std::vector<cv::Point3f> & point_3d, std::vector<cv::Point2f> & point_2d,
  std::vector<cv::Point2f> & convex_hull, double near, double far, double back)
{
  return get_landmarks_in_frustum(
    camera_frame->get_cam_pose(), camera_frame->camera_, camera_frame->get_map_id(),
...
void map_database::add_erased_server_landmark(LandmarkID id)
{
  ...
  if (server_landmarks_.count(id)) {
    server_landmarks_[id]->set_to_be_erased();
  }
}
...
{
  spdlog::debug(
    "Start to remove {} to_be_erased_server_landmarks!", to_be_erased_server_landmark_id_.size());
  std::unique_lock<std::mutex> lock(mtx_map_access_);
...
void map_database::remove_old_map()
{
  ...
  for (auto keyframe_id : erased_keyframes_id) {
    // Here has some problem, so we don't erase it
    // keyframes_[keyframe_id]->prepare_for_erasing(false);
    // now
    ...
    //   next_imu_constraint->merge_preintegration(
    //     keyframes_.at(keyframe_id)->get_imu_constraint().second);
    //   keyframes_.at(keyframe_id)->next_keyframe_->set_imu_constraint(
    //     keyframes_.at(keyframe_id)->get_imu_constraint().first, next_imu_constraint);
    ...
    //       keyframes_.at(keyframe_id)->pre_keyframe_;
    //     keyframes_.at(keyframe_id)->pre_keyframe_->next_keyframe_
...
void map_database::save_map_to_jsonfile(
...
void map_database::register_keyframe(
  camera_database * cam_db, bow_vocabulary * bow_vocab, bow_database * bow_db, const KeyframeID id,
  const nlohmann::json & json_keyfrm)
{
  if (is_server_node) {
    register_keyframe_server(cam_db, bow_vocab, bow_db, id, json_keyfrm);
  } else {
    register_keyframe_tracker(cam_db, bow_vocab, bow_db, id, json_keyfrm);
  }
}

void map_database::register_keyframe_tracker(
  camera_database * cam_db, bow_vocabulary * bow_vocab, bow_database * bow_db, const KeyframeID id,
  const nlohmann::json & json_keyfrm)
{
  // Metadata
  ...
  auto keyfrm = new data::keyframe(
    id, src_frm_id, timestamp, cam_pose_cw, camera, depth_thr, num_keypts, keypts, undist_keypts,
    bearings, stereo_x_right, depths, descriptors, num_scale_levels, scale_factor, bow_vocab,
    bow_db, this);

  // Append to map database
  assert(!keyframes_.count(id));
  std::scoped_lock lock(mtx_map_access_);
  keyframes_[keyfrm->id_] = keyfrm;
  if (keyfrm->id_ > max_keyfrm_id_) {
    max_keyfrm_id_ = keyfrm->id_;
  }
...
void map_database::register_keyframe_server(
  camera_database * cam_db, bow_vocabulary * bow_vocab, bow_database * bow_db, const KeyframeID id,
  const nlohmann::json & json_keyfrm)
{
  // Metadata
...
bool map_database::is_keyframe_loaded(keyframe * keyfrm) const
{
  ...
  std::scoped_lock<std::mutex> lock(mtx_map_access_);
  if (!loaded_keyframes_id_.count(keyfrm->client_id_)) {
    return false;
  }

  if (!loaded_keyframes_id_.at(keyfrm->client_id_).count(keyfrm->id_)) {
    return false;
  }
...
void map_database::register_landmark_tracker(
  const LandmarkID id, const nlohmann::json & json_landmark)
{
  const auto first_keyfrm_id = std::stoul(json_landmark.at("1st_keyfrm").get<std::string>());
  const auto pos_w =
    Vec3_t(json_landmark.at("pos_w").get<std::vector<Vec3_t::value_type>>().data());
  ...
  auto lm =
    new data::landmark(id, first_keyfrm_id, pos_w, ref_keyfrm, num_visible, num_found, this);
  assert(!landmarks_.count(id));
  landmarks_[lm->id_] = lm;
}

void map_database::register_landmark_server(
  const LandmarkID id, const nlohmann::json & json_landmark)
{
  ...
  assert(!landmarks_.count(id));
  auto lm = new data::landmark(
    id, first_keyfrm_id, pos_w, ref_keyfrm, num_visible, num_found, this, client_id, map_id);
  assert(!landmarks_.count(id));
  landmarks_[lm->id_] = lm;
...
bool map_database::is_landmark_loaded(landmark * landmark) const
{
  ...
  std::scoped_lock<std::mutex> lock(mtx_map_access_);
  if (!loaded_landmarks_id_.count(landmark->client_id_)) {
    return false;
  }

  if (!loaded_landmarks_id_.at(landmark->client_id_).count(landmark->id_)) {
    return false;
  }
...
void map_database::register_graph_server(const KeyframeID id, const nlohmann::json & json_keyfrm)
{
  ...
  const auto spanning_children_ids =
    json_keyfrm.at("span_children").get<std::vector<std::string>>();
  ...
  for (const auto & spanning_child_id : spanning_children_ids) {
    ...
    keyframes_.at(id)->graph_node_->add_spanning_child(
      keyframes_.at(std::stoul(spanning_child_id)));
  }
  ...
  if (spanning_parent_id == "-1") {
    ...
    spdlog::info(
      "In the loaded map, Map {}, Client {}: Origin keyframe {}", kf->get_map_id(), kf->client_id_,
      id);
    kf->set_origin();
    if (
      !origin_keyfrms_.count(kf->get_map_id()) ||
      origin_keyfrms_[kf->get_map_id()]->timestamp_ > kf->timestamp_) {
  ...
  if (front_and_rear_constraint_keyframe_id_string != "-1") {
    if (!keyframes_.count(std::stoul(front_and_rear_constraint_keyframe_id_string))) {
      ...
      keyframes_.at(id)->graph_node_->add_constraint_edge(
        std::stoul(front_and_rear_constraint_keyframe_id_string), kf);
    }
  }
}
...
void map_database::register_graph_tracker(const KeyframeID id, const nlohmann::json & json_keyfrm)
{
  ...
  keyframes_.at(id)->graph_node_->set_spanning_parent(
    (spanning_parent_id == -1) ? nullptr : keyframes_.at(spanning_parent_id));
  for (const auto spanning_child_id : spanning_children_ids) {
...
void map_database::register_association(
  const KeyframeID keyfrm_id, const nlohmann::json & json_keyfrm)
{
  if (is_server_node) {
...
void map_database::register_association_tracker(
  const KeyframeID keyfrm_id, const nlohmann::json & json_keyfrm)
{
...
void map_database::register_association_server(
  const KeyframeID keyfrm_id, const nlohmann::json & json_keyfrm)
{
  // Key points information
  const auto num_keypts = json_keyfrm.at("n_keypts").get<unsigned int>();
