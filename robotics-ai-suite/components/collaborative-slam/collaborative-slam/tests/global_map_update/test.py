...
"""
 Req: ASREQ-794 Collaborative SLAM: Local map update on global map updat
 Jira ID: SL6-209
 The test verifies the following
 1. Start serve
 2. Start tracker 1 and tracker 2 in the same tim
 3. Play 2 bags containing a common sectio
 4. Read topic "ros2 topic echo /univloc_tracker_0/pose" for Tracker 1 an
    watch its update as the bags simulate the 2 robots moving
 Test execution example
 python3 -m atf.atf test --test amr_tests/tests/collaborative_slam/global_map_updat
"""
...
class AtfTest(AMRTest):
    ...
    def execute(self: AMRTest):
        ...
        self.testing_targets[0].run_cmd(
            ...
            "vocabulary:=/opt/ros/${ROS_DISTRO}/share/univloc_tracker/config/orb_vocab.dbow2 "
            "log_level:=trace > /tmp/log-tracker0.txt &",
            terminal_id=self.testing_targets[0].term3,
            sleep=10,
            check_code=True,
        )
        self.testing_targets[0].run_cmd(
            "cat /tmp/log-tracker0.txt |"
            r" grep '\[INFO\] \[univloc_tracker_ros-1\]\: process started with pid'",
        ...
        self.testing_targets[0].run_cmd(
            "ros2 topic hz /univloc_tracker_2/pose > /tmp/log_pose2.txt &",
            terminal_id=self.testing_targets[0].term5,
            check_code=False,
        )

        # read by log-tracker2
        self.testing_targets[0].run_cmd(
        ...
        self.testing_targets[0].run_cmd(
            "cat /tmp/log-tracker0.txt |"
            ' grep "This message is for updating all local map! merge_times"',
    ...
    def cleanup(self, success):
        """Cleanup for global map update test."""
