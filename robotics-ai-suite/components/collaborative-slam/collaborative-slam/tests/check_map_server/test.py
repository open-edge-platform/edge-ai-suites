...
"""
    Req: ASREQ-792 Collaborative SLAM: Local map update on sensor input
    Req: ASREQ-943 Collaborative SLAM Microservices
    Jira ID: SL6-1147

    The test verifies the following:
    Launches the server
    Launches the tracker
    Checks in rviz, in server side, that a map is show
    ...
    python3 -m atf.atf test --test amr_tests/tests/collaborative_slam/check_map_server
"""
import os
...
class AtfTest(AMRTest):
    """Test class for check map server test."""
    def setup(self: AMRTest):
        """Setup for check map server test."""
    ...
    def execute(self: AMRTest):
        ...
        self.open_container_terminal(
            self.testing_targets[0], self.testing_targets[0].term2, env_display=":11.0"
        )
        ...
        self.testing_targets[0].run_cmd(
            cmd="Xvfb :11 -ac -screen 0 1680x1050x24 & disown; "
            "sleep 2 ; ps ax | grep Xvfb | grep -v grep",
            terminal_id=self.testing_targets[0].term1,
        )

        self.testing_targets[0].run_cmd(
            'sed -i "s/Height: 1648/Height: 589/" /opt/ros/${ROS_DISTRO}/share/univloc_server/config/rviz2_server.rviz',  # noqa: E501
            terminal_id=self.testing_targets[0].term2,
        )
        self.testing_targets[0].run_cmd(
            'sed -i "s/Width: 2192/Width: 861/" /opt/ros/${ROS_DISTRO}/share/univloc_server/config/rviz2_server.rviz',  # noqa: E501
            terminal_id=self.testing_targets[0].term2,
        )
        self.testing_targets[0].run_cmd(
            'sed -i "s/X: 104/X: 0/" /opt/ros/${ROS_DISTRO}/share/univloc_server/config/rviz2_server.rviz',  # noqa: E501
            terminal_id=self.testing_targets[0].term2,
        )
        self.testing_targets[0].run_cmd(
            'sed -i "s/Y: 60/Y: 0/" /opt/ros/${ROS_DISTRO}/share/univloc_server/config/rviz2_server.rviz',  # noqa: E501
            terminal_id=self.testing_targets[0].term2,
        )
        ...
        self.testing_targets[0].run_cmd(
            ...
            "gui:=false rviz:=true > /tmp/log-server1.txt &",
            terminal_id=self.testing_targets[0].term1,
            sleep=10,
            check_code=True,
        )
        ...
        # we take a screenshot for the server rviz (term1) and one for tracker rviz (term2).
        # we check that each of them contains the map displayed
        ...
        screenshot_remote_path = self.take_screenshot(
            screenshot_name, self.testing_targets[0], self.testing_targets[0].term1
        )
        ...
        if self.is_same_image(reference_negative_local_path, screenshot_remote_path, 0.9):
            return False

        screenshot_remote_path = self.take_screenshot(
            screenshot_name, self.testing_targets[0], self.testing_targets[0].term2
        )
        self.testing_targets[0].run_cmd(cmd="exit", terminal_id=self.testing_targets[0].term2)
        self.testing_targets[0].run_cmd(
            cmd="docker cp %s:%s %s"
            % (self.test_name, screenshot_remote_path, screenshot_remote_path),
            terminal_id=self.testing_targets[0].term2,
        ...
        screenshot_local_full_path = os.path.join(self.local_test_output_dir, screenshot_name)
        return not self.is_same_image(reference_negative_local_path, screenshot_remote_path, 0.9)

    def cleanup(self, success):
        """Cleanup for check map server test."""
