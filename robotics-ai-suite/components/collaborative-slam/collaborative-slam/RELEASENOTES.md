...
## Robotics AI suite - Release EAS 2026.0

* Added a memory-safe packaging path via `make safe-package` for resource-constrained systems, including automatic parallel job selection and support for manually overriding job count.

* Improved build flexibility and robustness by propagating proxy environment variables, extending artifact cleanup to include additional package outputs, and refining package cleanup behavior.
...
* Refined CI and security scan workflows with improved submodule handling, updated Trivy installation behavior/versioning, and corrected CI secret usage for LFS credentials.

* Updated Coverity integration behavior in packaging flow, including package-count expectations when Coverity mode is enabled.

* Synchronized documentation with Open Edge Platform guidance and expanded build/troubleshooting guidance for memory planning and oneAPI/SYCL compatibility.

* Applied minor maintenance cleanups in source and benchmark documentation, including spelling/comment corrections and small wording updates.
