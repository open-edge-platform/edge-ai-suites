...
#define ROS2_JAZZY_OR_NEWER
#endif

// Universal Eigen formatter
#include <sstream>
#include <Eigen/Dense>

template <typename Derived>
std::string eigen_str(const Eigen::MatrixBase<Derived> & matrix)
{
...
static const std::vector<Color> all_colors = {
  ...
  Color{"pink", 255, 192, 203}, Color{"yellow", 255, 255, 0}, Color{"hot_pink", 255, 105, 180},
  Color{"orange", 255, 165, 0}, Color{"purple", 128, 0, 128}, Color{"orange_red", 255, 59, 0},
  Color{"gold", 255, 215, 0}, Color{"magenta", 255, 0, 255}, Color{"lime", 0, 128, 0},
...
Server::Server(
  Camera_database * cam_db, Bow_vocabulary * bow_vocab, Bow_database * bow_db,
  Map_database * p_map_db_all, rclcpp::Node * node_ptr,
  ...
  cam_db_(cam_db)
{
  ...
  if (cfg_->server_mode_ == "mapping") {
    ...
    throw std::invalid_argument(
      "Available modes: mapping, localization, relocalization, remapping");
  }
  ...
  this->timer_ = pnh_->create_wall_timer(
    std::chrono::duration<double>(visualization_interval_),
    std::bind(&univloc_server::Server::ros_publisher, this));
  ...
  keypoints_publisher_ =
    pnh_->create_publisher<visualization_msgs::msg::MarkerArray>("~/keypoints", map_qos);
  keyframes_publisher_ =
    pnh_->create_publisher<visualization_msgs::msg::MarkerArray>("~/keyframes", map_qos);
  camera_view_publisher_ =
    pnh_->create_publisher<visualization_msgs::msg::MarkerArray>("~/camera_view", map_qos);
  ...
  comm_service_ = pnh_->create_service<univloc_msgs::MapBasedComm>(
    "tracker2server",
    std::bind(
      &Server::service_message_callback, this, std::placeholders::_1, std::placeholders::_2),
  ...
  if (delay_time_record_folder_ != "") {
    if (check_folder_path(delay_time_record_folder_)) {
      ...
      throw std::runtime_error(
        "Cannot use " + delay_time_record_folder_ + " as delay_time_record_folder");
    }
  }
  ...
  if (
    server_mode_ == server_mode_t::Localization || server_mode_ == server_mode_t::Relocalization ||
    server_mode_ == server_mode_t::Remapping) {
...
void Server::connect_tracker(ClientID client_id)
{
  if (comm_client_map_.find(client_id) == comm_client_map_.end()) {
    spdlog::error(
      "failed to retrieve client {} in comm_client_map_ when connecting to tracker.", client_id);
    return;
  }
...
MapID Server::get_running_client_map_id(Keyframe * keyfrm) const
{
  if (p_all_map_db_->is_keyframe_loaded(keyfrm)) {
    spdlog::debug(
      "The keyframe {} with client id {} is loaded from previous map!", keyfrm->id_,
      keyfrm->client_id_);
...
void Server::service_message_callback(
  const std::shared_ptr<univloc_msgs::MapBasedComm::Request> request,
  std::shared_ptr<univloc_msgs::MapBasedComm::Response> response)
{
  ...
  univloc_msgs::MapConstPtr map_msg_ptr =
    std::make_shared<univloc_msgs::Map>(std::move(request->req_map));
...
void Server::process_service_message(Map_Msg_ConstPtr map_msg_ptr)
{
  ...
  if (handlers_.find(client_id) == handlers_.end()) {
    handlers_.emplace(
      std::piecewise_construct, std::forward_as_tuple(client_id),
  ...
  if (map_msg_ptr->connecting_request) {
    ...
    connecting_threads_[client_id] =
      std::make_unique<std::thread>(&Server::connect_tracker, this, client_id);
  ...
  if (map_msg_ptr->is_octree_map_loaded) {
    ...
    if (loaded_octree_) {
      spdlog::warn(
        "The loaded octree map should only be sent once from tracker to server! \
                          The current loaded octree map is sent from client {}",
        client_id);
    } else if (
      server_mode_ == server_mode_t::Localization || server_mode_ == server_mode_t::Remapping) {
      loaded_octree_ = std::make_shared<se::Octree<SE_FIELD_TYPE>>();
      mapmsg_to_param_and_octree(
        map_msg_ptr, loaded_octree_, loaded_intrinsics_, loaded_octree_cfg_);
      create_octree_related_publishers();
    }
    return;
  } else if (server_mode_ == server_mode_t::Remapping && !loaded_octree_) {
    spdlog::warn(
      "Server won't process any message before loading octree map from tracker in remapping mode");
  }
  ...
  if (delay_time_record_folder_ != "") {
    static std::string record_filename = delay_time_record_folder_ + "/" +
                                         std::to_string((int)rclcpp::Clock().now().seconds()) +
                                         ".txt";
    ...
    file_init = false;
    record_delaytime << time_delay << "\t" << map_msg_ptr->keyframes.size() << "\t"
...
void Server::request_octree_from_clients(const std::set<ClientID> & client_ids)
{
  ...
  for (auto & id_client : handlers_) {
    if (client_ids.count(id_client.first)) {
      ...
      spdlog::info(
        "[{}]: Send request to client {} for its Octree map!", __func__, id_client.first);
    }
  }
...
void Server::mapmsg_to_param_and_octree(
  const Map_Msg_ConstPtr map_msg_ptr, std::shared_ptr<se::Octree<SE_FIELD_TYPE>> octree,
  ...
  struct fast_mapping::se_cfg & octree_cfg)
{
  ...
  // std::cout << "Origin Position: " << loaded_octree_->get_origin_position().transpose()
  //           << std::endl;
  // std::cout << "Octree Size: " << loaded_octree_->size() << std::endl;
  // std::cout << "Octree Dim: " << loaded_octree_->dim() << std::endl;
  // std::cout << "Octree Block Buffer Size: " << loaded_octree_->getBlockBuffer().size()
  //           << std::endl;
  // std::cout << "Octree Node Buffer Size: " << loaded_octree_->getNodesBuffer().size()
  //           << std::endl;

  auto end = now();
  spdlog::debug(
    "Construct octree and its related parameters from mapmsg took {} ms", duration_ms(end - start));
}
...
void Server::transform_voxel_and_add_to_merged_octree(
  std::shared_ptr<se::Octree<SE_FIELD_TYPE>> merged_octree,
  const Eigen::Vector3f & pos_world_to_transform, const Eigen::Matrix4d & Tww_aligned_ref,
  const se::Node<SE_FIELD_TYPE>::value_type & origin_voxel_data,
  const std::vector<std::array<int, 4>> & leaf_node_region, const int pose_id, bool is_leaf_node,
  const int side, const float min_acceptable_z)
{
  ...
  auto is_voxel_within_leaf_node_region = [&leaf_node_region](const Eigen::Vector3i & v) {
    for (const auto & region : leaf_node_region) {
      if (
        (v.x() >= region[0]) && (v.x() < region[1]) && (v.y() >= region[2]) &&
        (v.y() < region[3])) {
        return true;
      }
    }
  ...
  // For the region in the xy plane covered by the leaf nodes of reference octree (we consider
  // as free space), we don't integrate voxels from leaf nodes of the octree to be aligned since
  // the free space is already set, so there is no need for extra operation which might instead
  // mess up the correct representation.
  if (is_leaf_node && is_voxel_within_leaf_node_region(pos_voxel_scaled)) {
    return;
  }
  ...
  if (!fast_mapping::within_map_boundary(merged_octree, pos_voxel_scaled, x_dir, y_dir, z_dir)) {
    ...
    spdlog::debug("octree expanded, new origin is {}", eigen_str(new_origin.transpose()));
  }
  ...
  auto is_voxel_valid = [size](const Eigen::Vector3i & v) {
    return (
      (v.x() >= 0) && (v.x() < size) && (v.y() >= 0) && (v.y() < size) && (v.z() >= 0) &&
      (v.z() < size));
  };
  ...
  if (is_leaf_node) {
    // For leaf node of octree to be aligned, we need to extend the range of [zmin, zmax] to avoid
    // missing data since leaf node's height range could have no intersection with [zmin, zmax].
    // Although the coordinate of the leaf node is within the [zmin, zmax], its sub-space can be
    // extended outside of the range. For zmin, extending by 1 voxel to accommodate for precision
    // loss during calculation, meanwhile, make sure larger than the minimum acceptable height
    // to avoid constructing map that includes floor; For zmax, extending by the leaf node's side
    // to include all the necessary information (its sub-space) needed.
    zmin =
      std::max(zmin - voxel_size * float(Z_MIN_EXTEND_VOXEL_NUM), min_acceptable_z - new_origin(2));
  ...
  if (!is_voxel_valid(pos_voxel_scaled)) {
    return;
  }

  // if the voxel's probability is below certain threshold, we will store traversed nodes to count
  // free voxels otherwise, there is no need to store traversed nodes
  se::VoxelBlock<SE_FIELD_TYPE> * new_voxel_block = nullptr;
  if (!is_leaf_node) {
    new_voxel_block =
      merged_octree->fetch(pos_voxel_scaled(0), pos_voxel_scaled(1), pos_voxel_scaled(2));
    if (!new_voxel_block) {
      new_voxel_block = merged_octree->insert(
        pos_voxel_scaled(0), pos_voxel_scaled(1), pos_voxel_scaled(2), pose_id);
    }
  } else {
    new_voxel_block =
      merged_octree->fetch_and_store_traversed_nodes(pos_voxel_scaled, traversed_nodes);
    if (!new_voxel_block) {
      new_voxel_block =
  ...
  if (!new_voxel_block) {
    spdlog::warn(
      "Fetched or inserted voxel block is nullptr, corresponding frame id is {}, pose is \n{}",
      pose_id, eigen_str(pos_voxel_scaled.transpose()));
    return;
  }
  ...
  int new_dataid = coords_offset(0) + coords_offset(1) * new_voxel_block->side +
                   coords_offset(2) * new_voxel_block->sideSq;
  ...
  if (int(new_voxel_data.y) == 0 && int(new_voxel_data.x) == 0) {
    ...
    spdlog::debug(
      "traversed_nodes size is {} after fetch or insert voxel block", traversed_nodes.size());
    for (auto & node : traversed_nodes) {
      node->free_voxel_cnt_ += 1;
    }
  }
  spdlog::debug("Finished transforming voxel and add to merged octree");
}

void Server::integrate_voxelblocks_to_merged_octree(
    ...
    for (int i = 0; i < block_side; ++i) {
      for (int j = 0; j < block_side; ++j) {
        for (int k = 0; k < block_side; ++k) {
          ...
          transform_voxel_and_add_to_merged_octree(
            merged_octree, pos_world, coordinate_transform, origin_voxel_data, leaf_node_region,
            pose_id, false);
        }
      }
    }
...
void Server::integrate_leaf_nodes_to_merged_octree(
  std::shared_ptr<se::Octree<SE_FIELD_TYPE>> octree_to_transform,
  std::shared_ptr<se::Octree<SE_FIELD_TYPE>> merged_octree,
  const Eigen::Matrix4d & coordinate_transform,
  const std::vector<std::array<int, 4>> & leaf_node_region)
{
  ...
  // since only the leaf nodes whose probability lower than 0 are used to update occupancy map when
  // publishing we will use the same data to merge octree map as well
  ...
  for (const auto & node : nodes) {
    int side = node.side;
    if (side > MAX_NODE_SIZE) {
      // If node side is too coarse, skip it.
      continue;
    }
    ...
    for (int i = 0; i < side; i++) {
      for (int j = 0; j < side; j++) {
        for (int k = 0; k < side; ++k) {
          ...
          transform_voxel_and_add_to_merged_octree(
            merged_octree, pos_world, coordinate_transform, origin_voxel_data, leaf_node_region,
            pose_id, true, side, min_acceptable_z);
        }
      }
    }
  }
  ...
  const std::shared_ptr<se::Octree<SE_FIELD_TYPE>> octree)
{
  ...
  std::shared_ptr<se::Octree<SE_FIELD_TYPE>> compact_octree =
    std::make_shared<se::Octree<SE_FIELD_TYPE>>();
  compact_octree->init(octree->size(), octree->dim(), octree->get_origin_position());
  spdlog::debug(
    "origin of processed octree is {}",
    eigen_str(compact_octree->get_origin_position().transpose()));
  ...
  auto valid_voxel_number_of_node = [voxel_size, inverse_voxel_size, zmin, zmax](int side, int z) {
    ...
    if (node_zmin > zmax || zmin > node_zmax) {
      return 0;
    }
  ...
  while (!st.empty()) {
    ...
    auto n = compact_octree->insert(
      coords(0), coords(1), coords(2), se::keyops::level(node->code_), node->pose_id_);
    ...
    if (
      node->free_voxel_cnt_ >
      static_cast<unsigned int>(
        FREE_VOXEL_PROPORTION_THRE * valid_voxel_number_of_node(node->side_, coords(2)))) {
  ...
  const Eigen::Matrix4d & coordinate_transform)
{
  ...
  spdlog::debug(
    "origin of merged octree is {}", eigen_str(merged_octree->get_origin_position().transpose()));
  ...
  auto is_node_intersect_with_remapping_region =
    [&](double xmin, double xmax, double ymin, double ymax) -> bool {
    struct fast_mapping::point_2d node_region_vertexes[4] = {
      {xmin, ymin}, {xmax, ymin}, {xmax, ymax}, {xmin, ymax}};
    auto node_region =
      std::make_unique<fast_mapping::remapping_region>(node_region_vertexes, false);
    if (remapping_region_->intersect_with_region(*node_region)) {
      return true;
    }
    return false;
  };
  ...
  for (; node != nullptr; node = it.next()) {
    if (node->isLeaf()) {  // voxelblock
      ...
      if (server_mode_ == server_mode_t::Remapping) {
        ...
        int side = node->side_;
        if (is_node_intersect_with_remapping_region(
              pos_world(0), pos_world(0) + voxel_size * float(side), pos_world(1),
              pos_world(1) + voxel_size * float(side))) {
      ...
      auto n = merged_octree->insert(
        coords(0), coords(1), coords(2), se::keyops::level(node->code_), node->pose_id_);
      std::memcpy(n->value_, node->value_, sizeof(node->value_));
    }
  }
  spdlog::debug(
    "Finished adding reference octree, intersect node cnt: {}, intersect voxelblock cnt: {}",
  ...
  integrate_voxelblocks_to_merged_octree(
    octree_to_transform, merged_octree, coordinate_transform, leaf_node_region);
  auto iterate_block_end = now();
  spdlog::debug(
    "Iterating voxelblocks of octree to be transformed and add to merged octree cost time {} ms",
  ...
  integrate_leaf_nodes_to_merged_octree(
    octree_to_transform, merged_octree, coordinate_transform, leaf_node_region);
  auto iterate_node_end = now();
  spdlog::debug(
    "Iterating nodes of octree to be transformed and add to merged octree cost time {} ms",
...
void Server::send_merged_octree_to_trackers()
{
  ...
  // for remapping mode, the reference octree is loaded and there will be only 1 map from
  // octree_from_trackers_
  for (const auto & idx : octree_from_trackers_) {
    std::optional<Eigen::Matrix4d> maybe_transform =
      handlers_.at(idx.first).get_coordinate_transform_matrix();
    if (!maybe_transform) {
      ref_octree = idx.second;
      spdlog::info(
        "reference octree map from client {}, origin is {}", idx.first,
        eigen_str(ref_octree->get_origin_position().transpose()));
    } else {
      octree_to_transform = idx.second;
      coordinate_transform = *maybe_transform;
      spdlog::info(
        "octree map to be aligned from client {}, origin is {}, coordinate transform is \n {}",
        idx.first, eigen_str(octree_to_transform->get_origin_position().transpose()),
        eigen_str(coordinate_transform));
  ...
  if (server_mode_ == server_mode_t::Remapping) {
    ...
    spdlog::info(
      "get reference octree map from loaded octree at remapping mode, origin is {}",
      eigen_str(ref_octree->get_origin_position().transpose()));
  }
  ...
  auto global_octree =
    merge_octree_from_trackers(ref_octree, octree_to_transform, coordinate_transform);
  auto merge_octree_end = now();
  spdlog::debug(
    "Merging two octree maps cost time {} ms", duration_ms(merge_octree_start, merge_octree_end));
...
void Server::send_nearby_landmarks_to_client(Keyframe * frame)
{
  ...
  if (frame->get_map_id() != current_frame->get_map_id()) {
    spdlog::debug(
      "Will skip sending nearby landmarks because client-{} has changed from map-{} to map-{}",
      frame->client_id_, frame->get_map_id(), current_frame->get_map_id());
    return;
  }

  spdlog::debug(
    "This frame id: {}, last frame id of relative client: {} ", frame->id_, current_frame->id_);
  {
    // TODO need to lock the map?
    // ros::Time start_time = ros::Time::now();
    auto start_time = now();
    std::vector<Mappoint *> nearby_mappoints = p_all_map_db_->get_landmarks_in_frustum(
      current_frame, point_3d, point_2d, convex_hull, near_distance_, far_distance_,
      back_distance_);

    if (nearby_mappoints.size() < nearby_landmarks_num_thr) {
      spdlog::debug(
        "{} less than {} nearby landmarks, so we will not send them", nearby_mappoints.size(),
    ...
    spdlog::debug(
      "Send {} nearby server landmarks to client {}", nearby_mappoints.size(), client_id);
  }
}
...
void Server::publish_camera_view(Keyframe * kf)
{
  ...
  static const std::vector<Eigen::Vector3d> line_strip_points = {
  ...
  std::map<std::string, visualization_msgs::msg::Marker> raycasting_markers, mappoint_markers,
    convex_markers_;
...
void Server::check_camera_fusion(Keyframe * keyframe)
{
  camera_fusion_module_->process_new_keyframe(keyframe);
}

void Server::save_client_camera(Map_Msg_ConstPtr map_camerainfo_msg)
{
  if (cam_db_->has_such_client_camera(map_camerainfo_msg->mn_client_id)) {
    return;
  }
  ...
  std::string camera_name =
    camerainfo_msg.camera_typename + std::to_string(map_camerainfo_msg->mn_client_id);
    ...
    switch (camera_model_type) {
      case Camera::model_type_t::Perspective: {
        camera = new Camera::perspective(
          camera_name, camera_setup_type, camera_color_order, camerainfo_msg.cols,
          camerainfo_msg.rows, camerainfo_msg.fps, (double)camerainfo_msg.fx,
          (double)camerainfo_msg.fy, (double)camerainfo_msg.cx, (double)camerainfo_msg.cy,
          (double)camerainfo_msg.k1, (double)camerainfo_msg.k2, (double)camerainfo_msg.p1,
          (double)camerainfo_msg.p2, (double)camerainfo_msg.k3, camerainfo_msg.focal_x_baseline);
      ...
      case Camera::model_type_t::Fisheye: {
        camera = new Camera::fisheye(
          ...
          (double)camerainfo_msg.fy, (double)camerainfo_msg.cx, (double)camerainfo_msg.cy,
          (double)camerainfo_msg.k1, (double)camerainfo_msg.k2, (double)camerainfo_msg.p1,
          (double)camerainfo_msg.p2, camerainfo_msg.focal_x_baseline);
        camera->show_parameters();
        cam_db_->add_client_camera(map_camerainfo_msg->mn_client_id, camera);
        break;
      }
  ...
  spdlog::debug(
    "~~~~~~~~~~~~~~~~~Successfully save camera from client {}~~~~~~~~~~~~~~~~~ ",
    static_cast<int>(map_camerainfo_msg->mn_client_id));
}

void Server::get_local_keyframes_and_mappoints(
  Keyframe * cur_keyframe, std::vector<Keyframe *> & local_keyframes,
  std::vector<Mappoint *> & local_mappoints)
{
  ...
  for (int i = 0; i < try_times; i++) {
    ...
    if (
      keyfrm && !keyfrm->will_be_erased() &&
      keyfrm->get_map_id() == map_id) {  // no need to judge client id
      local_keyframes.push_back(keyfrm);
    }

    if (id == 0 || local_keyframes.size() >= max_local_keyframes_size) {
      break;
    }
  }
...
void Server::send_updatedstate_to_client(ClientID client_id, bool is_loop_closure)
{
  ...
  std::optional<Eigen::Matrix4d> coordinate_transform =
    handlers_.at(client_id).get_coordinate_transform_matrix();
  if (coordinate_transform) {
    spdlog::warn(
      "The world coordinate of the reference map to the world coordinate of the map to be aligned "
      "is \n {}",
      eigen_str(*coordinate_transform));
...
void Server::post_process_after_loop(ClientID, ClientID, MapID map1, MapID map2)
{
  ...
  if (map1 != map2) {
    ...
    if (origin_keyfrm_max) {
      ...
      spdlog::warn(
        "Erase origin keyframe {} of client {} from the unregistered map {}!",
        origin_keyfrm_max->id_, origin_keyfrm_max->client_id_, larger_map_id);
    }
    ...
    if (origin_keyfrm_max) {
      ...
      spdlog::info(
        "Set spanning parent for keyframe {} to the origin keyframe {}!", origin_keyfrm_max->id_,
...
void Server::send_msg_to_client(ClientID client_id, univloc_msgs::Map & msg_to_be_sent)
{
  ...
  spdlog::debug(
    "Send {} updated keyframe message to client: {}", msg_to_be_sent.keyframes.size(), client_id);
  if (unlikely(msg_to_be_sent.landmarks.empty())) {
    if (!msg_to_be_sent.is_request_octree && !msg_to_be_sent.is_merged_octree) {
      ...
      spdlog::debug(
        "Send merged octree nodes {} voxel blocks {} to client {}",
        msg_to_be_sent.octree[0].nodes.size(), msg_to_be_sent.octree[0].blocks.size(), client_id);
    }
  } else {
    spdlog::debug(
      "Send {} updated landmarks message to client: {}", msg_to_be_sent.landmarks.size(),
      client_id);
  }
...
void Server::convert_mappoints_to_msg(
  std::vector<Mappoint *> mappoints, univloc_msgs::Map & msg_out, bool only_send_extra)
{
  // ros::Time start = ros::Time::now();
  auto start = now();
  ...
  for (uint i = 0; i < size; i++) {
    ...
    mappoint_msg.mn_observed_in_other_keyframe_index =
      p_cur_mappoint->observed_in_other_keyframe_index_;
    p_cur_mappoint->replace_other_id_ = 0;
    p_cur_mappoint->observed_in_other_keyframe_id_ = 0;

    // If the last landmark observation is far behind current frame, it means this landmark is not
    // observed by current frame, so we need to send its descriptor Has problem , we need to find
    // out those landmarks which may not in the client local map, if sent not correctly, it may
    // cause a mistake, so we send this msg anyway
    // if (p_cur_mappoint->get_identifier_last_observation_id() != cur_frame_id) {
    ...
    if (descriptors_cols != mappoint_msg.mv_descriptors[0].m_descriptor.size()) {
      spdlog::warn(
        "descriptor of landmark {} has wrong size {}", p_cur_mappoint->id_, descriptors_cols);
      continue;
    }
  ...
  auto finish = now();
  spdlog::debug(
    "convert {} mappoints to msg consumes: {} ms.", msg_out.landmarks.size(),
    duration_ms(finish - start));
}

void Server::convert_keyframes_to_msg(
  std::vector<Keyframe *> keyframes, univloc_msgs::Map & msg_out, ClientID client_id)
{
  // ros::Time start = ros::Time::now();
  auto start = now();
  ...
  for (uint i = 0; i < size; i++) {
    ...
    if (p_all_map_db_->is_keyframe_loaded(cur_keyframe)) {
      spdlog::debug(
        "keyframe {} is loaded from previous map, won't convert to msg!", cur_keyframe->id_);
      continue;
    }
    if (client_id != keyframe_client_id) {
      spdlog::debug(
        "keyframe {} belongs to client {}, not {}, won't convert to msg!", cur_keyframe->id_,
    ...
    keyframe_msg.mn_client_id = static_cast<ClientID>(keyframe_client_id);
    {
      ...
      for (int mi = 0; mi < 4; ++mi) {
        for (int mj = 0; mj < 4; ++mj) {
          keyframe_msg.mv_pose[mk++] = pose(mi, mj);
        }
      }
    }
  ...
  auto finish = now();
  spdlog::debug(
    "Converting {} keyframes to msg consumes {} s", msg_out.keyframes.size(),
    duration_ms(finish - start));
...
void Server::pause_process_msg(ClientID client_id, bool wait_until_paused)
{
  ...
  while (!handlers_.at(client_id).is_paused()) {
...
void Server::set_global_optimization_module(
  openvslam::univloc_global_optimization_module * global_optimizer)
{
...
{
  if (keyframes_publisher_->get_subscription_count() == 0) {
    return;
  }
  ...
  // Previously, we use line strip points (p1-p2-p3-p4-p5-p6-p7...pN) to draw each keyframe
  // and to connect to the next. This will cause the simliar issue like
  // https://github.com/ros-visualization/rviz/issues/1107. So we decide
  // to use line list point (p1-p2,p2-p3,p3-p4,p4-p5...pN) to do the same thing.
  static const std::vector<Eigen::Vector3d> line_list_points = {
    Eigen::Vector3d(0, 0, 0),
    Eigen::Vector3d(-half_width, -half_height, offset),
    Eigen::Vector3d(-half_width, half_height, offset),
    Eigen::Vector3d(-half_width, half_height, offset),
  ...
  for (auto & it : handlers_) {  // for each client
    ...
    const std::vector<Keyframe *> session_end_keyframes =
      it.second.get_latest_keyframe_of_all_sessions();
    ...
    for (size_t session_id = 0; session_id < session_size; ++session_id) {  // for each session
      ...
      for (const auto & id_keyframe : all_keyframes) {
        if (
          id_keyframe.second->get_map_id() == end_frame->get_map_id() &&
          id_keyframe.second->client_id_ == client_id)
          keyframes_map.insert(
            std::pair<KeyframeID, Keyframe *>(id_keyframe.first, id_keyframe.second));
      }

      std::map<KeyframeID, Keyframe *>::reverse_iterator it = keyframes_map.rbegin();
      // The number of line list points must be even, so we need to insert a point at the beginning
      // and end of it to connect the centers of adjacent keyframes.
      ...
      while (it != keyframes_map.rend()) {
        ...
        for (const auto & loop_keyframe : loop_keyframes) {
          std::string ns = "loop-" +
                           std::to_string(std::min(client_id, loop_keyframe->client_id_)) + "-" +
          ...
          Eigen::Vector3d loop_keyframe_position =
            loop_keyframe->get_cam_pose_inv().block(0, 3, 3, 1);
          geometry_msgs::msg::Point point;
        ...
        if (frame->graph_node_->has_front_rear_camera_constraint()) {
          ...
          if (kf->get_map_id() == frame->get_map_id() && kf->client_id_ < frame->client_id_) {
            ...
            Eigen::Vector3d multi_camera_keyframe_position =
              kf->get_cam_pose_inv().block(0, 3, 3, 1);
            geometry_msgs::msg::Point point;
            point.x = Twc.translation()[0];
            point.y = Twc.translation()[1];
            point.z = Twc.translation()[2];
      ...
      if (session_marker.points.size() == 0) {
        continue;
      }
  ...
  for (const auto & item : loaded_keyframes_pos_inv) {
    ...
    if (loaded_marker.points.size() == 0) {
      continue;
    }
    ...
    ss << "Loaded client-" << static_cast<int>(client_id) << "  " << item.second.size()
       << " keyframes /n";
  }
...
{
  if (pub_octree_map_->get_subscription_count() == 0) {
    return;
  }
  ...
  auto set_marker_color =
    [](visualization_msgs::msg::Marker & marker, float r, float g, float b, float a = 1.0) {
      marker.color.r = r;
      marker.color.g = g;
      marker.color.b = b;
      marker.color.a = a;
  ...
  if (server_mode_ == server_mode_t::Remapping) {
    ...
    for (unsigned int i = 0; i < size; i++) {
      struct fast_mapping::point_2d voxel_2d_point = {
        fused_marker->points[i].x, fused_marker->points[i].y};
...
{
  if (pub_occupancy_map_->get_subscription_count() == 0) {
    return;
  }
  ...
  if (octree_size != grid_template_->info.width) {
    ...
    uint64_t data_size = (uint64_t)og.info.width * (uint64_t)og.info.height;
    og.data.resize(data_size, fast_mapping::unknown_prob);
    int dx = std::nearbyint(
      (grid_template_->info.origin.position.x - og.info.origin.position.x) / og.info.resolution);
    int dy = std::nearbyint(
      (grid_template_->info.origin.position.y - og.info.origin.position.y) / og.info.resolution);
    if (dx < 0 && dy < 0) {
  ...
  for (const auto & block : blocks) {
    ...
    for (int i = 0; i < block_side; ++i) {
      for (int j = 0; j < block_side; ++j) {
        ...
        if (vx >= (int)og.info.width || vy >= (int)og.info.height) {
          break;
        }
        ...
        for (int k = 0; k < block_side; ++k, center.z += voxel_dim, dataid += block->sideSq) {
          ...
          if (prob > fast_mapping::unknown_upper) {
            ogdata = fast_mapping::occupied_prob;  // Occupied
            ...
            if (ogdata == fast_mapping::unknown_prob) {
              ogdata = fast_mapping::free_prob;
            }
          }
        }
      }
...
void Server::octree_sync_callback(const std_msgs::msg::String::SharedPtr msg)
{
  if (msg->data == "octree_sync") {
    ...
    spdlog::warn(
      "Got {} message over ROS octree sync topic, but did not match required string!", msg->data);
  }
}
...
std::unordered_map<ClientID, std::shared_ptr<se::Octree<SE_FIELD_TYPE>>> &
Server::get_octree_from_trackers()
{
...
std::unordered_map<ClientID, struct fast_mapping::camera_intrinsics> &
Server::get_octree_intrinsic_from_trackers()
{
...
void Server::create_octree_related_publishers()
{
  auto map_qos = rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable();
  pub_octree_map_ =
    pnh_->create_publisher<visualization_msgs::msg::MarkerArray>("~/fused_map", map_qos);
...
bool Server::remove_pointcloud_within_remapping_region()
{
  if (!p_all_map_db_ || !remapping_region_) {
    spdlog::error(
      "Map database or remapping region hasn't been constructed before removing pintcloud!");
    return false;
  }
  ...
  spdlog::info(
    "Totally removed {} landmarks and {} keyframes within the remapping region!",
...
void Server::remove_clients_pointcloud_outside_remapping_region()
{
  if (!p_all_map_db_ || !remapping_region_) {
    spdlog::error(
      "Map database or remapping region has been destructed before removing pintcloud!");
    return;
  }
  ...
  for (auto & landmark : all_landmarks) {
    Eigen::Vector3d lm_pose = landmark->get_pos_in_world();
    if (
      !p_all_map_db_->is_landmark_loaded(landmark) &&
  ...
  for (auto & keyframe : all_keyframes) {
    ...
    Eigen::Vector3d keyfrm_pose = keyfrm_Twc.block<3, 1>(0, 3);
    if (
      !p_all_map_db_->is_keyframe_loaded(keyframe) &&
  ...
  // Print log info fot the debugging
  spdlog::info(
    "Totally removed {} client landmarks and {} client keyframes outside the remapping region!",
    removed_landmarks.size(), removed_keyframes.size());
}

void Server::set_coordinate_transform(
  ClientID client_id_for_map_to_be_aligned, const Eigen::Matrix4d transform_ww_aligned_ref)
{
  std::optional<Eigen::Matrix4d> Tww_aligned_ref =
    std::make_optional<Eigen::Matrix4d>(transform_ww_aligned_ref);
