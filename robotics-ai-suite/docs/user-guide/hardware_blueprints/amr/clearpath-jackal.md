...
The Clearpath Robotics Jackal is a validated Autonomous Mobile Robot
configuration with a RealSense D435i camera.
...
1. To install the ROS 2 distribution and the Autonomous Mobile Robot on the
Clearpath Robotics Jackal robot, see the
[Getting Started](../../platform_foundation/getting_started.md) guide of the Autonomous Mobile Robot.

2. Create an account with the username ``administrator``when
...
2. Stop the command by pressing ``Ctrl-c``.
...
2. You can use the example configuration
...
3. Copy the ``robot.yaml`` file
   to the ``/etc/clearpath/`` folder on the onboard computer of your
   Jackal robot.

4. If your ``~/.bashrc`` script defines a ``ROS_DOMAIN_ID`` environment variable,
...
2. Copy the ``robot.yaml`` file to the ``/etc/clearpath/`` folder on the onboard computer
   of your Jackal robot.

3. Reboot the robot to propagate the new configuration.
...
Ensure that you have set up your Jackal robot as described in the
[Set Up the Jackal](#set-up-the-jackal) section.
...
1. Test whether the Clearpath Robotics services are running on your robot:
   ...
   See the [Jackal Troubleshooting](#jackal-troubleshooting)
...
4. You can also manually publish to the ``/cmd_vel`` topic to move the robot.
