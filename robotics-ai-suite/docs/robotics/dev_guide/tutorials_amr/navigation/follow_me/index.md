...
- **Hand gesture of the target:** An open-source deep learning model, developed by Google's Mediapipe framework, is used for the target person's hand gesture recognition from RGB image.
  See [Mediapipe Hands](https://mediapipe.readthedocs.io/en/latest/solutions/hands.html) for more details of the model. `ros-humble-gesture-recognition-pkg` subscribes to the RGB image and publishes ROS 2 topic with gesture category `msg`.
- **Voice command:** A pre-trained neural network, called Quartznet, is used for automatic speech recognition and convert the target person's voice commands into robot motion control commands.
  The pre-trained [Quartznet](https://docs.openvino.ai/2023.3/omz_models_model_quartznet_15x5_en.html) parameters are obtained from OpenVINO™ model zoo. `ros-humble-speech-recognition-pkg` launches this node and publishes ROS 2 topic with voice-based robot control commands.
...
of the robot's activity during the course of its movement. This module uses pre-trained text-to-speech synthesis model, called [Forward-Tacotron](https://docs.openvino.ai/2023.3/omz_models_model_forward_tacotron.html) from OpenVINO™ model zoo.
